...
public class GeminiService {
    ...
    public ChatResponse processChat(ChatRequest request, String username) {
        ...
        contents.add(new Content("model", java.util.Arrays.asList(new Part("Vâng, tôi đã hiểu. Tôi sẽ đóng vai bác sĩ da liễu DermaCare và hỗ trợ bệnh nhân dựa trên dữ liệu này."))));

        // Append recent chat history (last 5 messages)
        List<ChatMessage> history = chatMessageRepository.findTop5BySessionUuidOrderByCreatedAtDesc(request.getSessionUuid());
        for (int i = history.size() - 1; i >= 0; i--) {
            ChatMessage msg = history.get(i);
            // Skip the one we just saved
            if (msg.getMessageId().equals(userMessage.getMessageId())) continue;
        ...
        try {
            GeminiResponse geminiRes = restTemplate.postForObject(GEMINI_URL + geminiApiKey, entity, GeminiResponse.class);
    ...
    private String buildPatientContext(User user) {
        ...
        if (patient == null) return "Không có dữ liệu bệnh nhân.";

        context.append("- Tiền sử bệnh: ").append(patient.getMedicalHistory() != null ? patient.getMedicalHistory() : "Không có").append("\n");
        ...
        if (latestAi != null) {
            context.append("- Kết quả AI phân tích ảnh gần nhất (").append(latestAi.getCreatedAt().toString()).append("):\n");
            context.append("  + Bệnh dự đoán 1: ").append(latestAi.getTop1Disease() != null ? latestAi.getTop1Disease().getDiseaseNameVi() : "N/A")
                   .append(" (Tỷ lệ: ").append(latestAi.getTop1Confidence()).append(")\n");
            context.append("  + Cảnh báo ác tính: ").append(latestAi.getHasCancerWarning() ? "CÓ" : "KHÔNG").append("\n");
        }

        // Get latest Medical Record
        MedicalRecord latestRecord = medicalRecordRepository.findTop1ByPatientOrderByExaminedAtDesc(patient).orElse(null);
